...
//instance index => prepend to panel
//trigger index => appended to trigger id and prepend panel id
//pass in menu count -> menu index depth index
...
TraversableMenu.prototype.panelInitialize = function (panel, depth, options) {
  try {
    ...
    this.panelIDSetByDepthIndex(panel, depth, this.menu_item_index); //inside here
    ...
    for (var i = 0; i < menu_items.length; i++) {
      ...
      if (child_panel) {
        ...
        if (this.depth_max_canonical === null || depth <= this.depth_max_canonical) {
          ...
          //
          var ret = this.panelInitialize(child_panel, depth, {
            panel_parent: panel,
            parent_menu_item: menu_items[i]
          });
          depth--;
...
TraversableMenu.prototype.triggerAttributesInit = function (trigger, panel, trigger_type) {
  ...
  try {
    ...
    trigger.setAttribute('id', this.elementIDPrefix() + '_trigger_' + trigger_type + '_' + panel_depth.toString() + '_' + panel_index.toString() + '_' + this.trigger_index.toString()); //add global trigger and instance index
  ...
  try {
    ...
    if (this.menu_item_active) {
      ...
      if (active_panel) {
        ...
        this.panelActivate(active_panel, {
          'show_immediate': true
        });
        return true;
      }
